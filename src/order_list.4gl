...
function init()
    let m_toggle = nvl(m_toggle, "code")
    call db_populate() 
    call ui_populate()
end function
...
function list()
    ...
    open window order_list with form "order_list"
    let w= ui.Window.getCurrent()
    let f = w.getform()
    #call f.loadToolBar("catalog_list")
    
    call db_populate() 
    call ui_populate()
    call ui_list()
    
    close window order_list
end function
...
define l_row integer
    ...
    case m_toggle
        when "code"                     
            ...
            let m_arr[l_row].minor = SFMT("%1 %2", m_order_header_arr[l_row].oh_order_date, m_order_header_arr[l_row].oh_cu_code)
            let m_arr[l_row].img = ""

...
type order_line_type record like order_line.*
define m_order_line_arr dynamic array of order_line_type -- used to display multiple rows
...
function list()
    ...
    open window order_line_list with form "order_line_list"
...
define i integer
    ...
    call m_arr.clear()
    for i = 1 to m_order_line_arr.getLength()
...
define l_row integer
    ...
    case m_toggle
        when "qty"                     
            let m_arr[l_row].major = m_order_line_arr[l_row].ol_pr_code
            let m_arr[l_row].minor = m_order_line_arr[l_row].ol_qty
            let m_arr[l_row].img = ""
...
define l_sql string
define l_rec order_line_type

    
    call m_order_line_arr.clear()
    ...
    foreach product_list_curs into l_rec.*
        let m_order_line_arr[m_order_line_arr.getlength()+1].* = l_rec.*
    end foreach
end function